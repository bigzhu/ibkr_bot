...
def demark(klines_data: list[Kline]) -> tuple[str, int, list[Kline]]:
    ...
    return _build_signal_payload(td_up, td_down, sequence_klines)
...
) -> tuple[str, int, list[Kline]]:
    if td_up and td_up > 0:
        return SELL, td_up, sequence_klines
    if td_down and td_down > 0:
        return BUY, td_down, sequence_klines
    return "NONE", 0, sequence_klines
